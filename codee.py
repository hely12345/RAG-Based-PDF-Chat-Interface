...
def split_chunks(text, chunk_size=500, overlap=50):
    ...
    if len(chunks) >= MAX_CHUNKS_PER_DOC:
        st.warning(
            f"Document truncated to {MAX_CHUNKS_PER_DOC} chunks due to size limits."
        )
...
with st.sidebar:
    ...
    if uploaded_files:
        ...
        if current_names != st.session_state.doc_names:
            ...
            if oversized:
                st.error(
                    f"Files too large: {', '.join(oversized)}. Max 10MB per file."
                )
            else:
...
if not st.session_state.index:
    ...
    if not st.session_state.messages:
        ...
        st.caption(
            f"Loaded **{len(st.session_state.doc_names)}** document(s)"
        )
